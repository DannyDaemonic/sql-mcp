//! sql-mcp: a deliberately tiny MCP server.
//!
//! One tool, `sql_exec(sql)`, and no helper tools — the SQL channel already
//! does introspection; the dialect-specific hints come from the driver and
//! are surfaced in the tool description. Credentials
//! live in one `0600` TOML file, never in environment variables. Optional
//! read-only mode refuses to start unless the account is provably read-only.
...
struct SqlExecArgs {
    /// The SQL statement to execute against the configured database.
    sql: String,
}
...
impl SqlServer {
    ...
    // The static description is a placeholder; `list_tools` replaces it with
    // `tool_description()`, which knows the backend dialect and the caps.
    ...
    async fn sql_exec(
        ...
        match self.driver.exec(&args.sql, self.limits).await {
            Ok(output) => {
                // Compact, not pretty: pretty-printing inflates every cell
                // with indentation the model pays context tokens for.
                let json = serde_json::to_string(&output).unwrap_or_else(|e| {
                    format!("{{\"error\":\"failed to serialize result: {e}\"}}")
                });
                Ok(CallToolResult::success(vec![Content::text(json)]))
            }
            // Surface SQL errors to the model as a tool error rather than failing
            // the whole call, so it can read the message and adjust.
    ...
    fn tool_description(&self) -> String {
        ...
        if self.limits.max_cell_bytes != 0 {
            caps.push(format!(
                "{} bytes per value (cut values end in …[truncated; N bytes total])",
        ...
        let caps = if caps.is_empty() {
            ...
            format!(
                " Limits: {}; truncated results include \"truncated\": true; cut strings \
                 end in …[truncated; N bytes total].",
                caps.join(", ")
            )
        };
        format!(
            "Run SQL against the configured {} database. Multiple statements are allowed. \
             ...
             For introspection, use SQL such as {}.",
            self.driver.name(),
...
impl ServerHandler for SqlServer {
    fn get_info(&self) -> ServerInfo {
        ...
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            ...
            .with_instructions(format!(
                "Runs SQL against a {} database via the single tool `sql_exec(sql)`. \
                 There are no helper tools — use SQL directly for introspection \
                 (e.g. {}).{mode}",
                self.driver.name(),
                self.driver.introspection_hint(),
            ))
    ...
    // Hand-written (instead of `#[tool_handler]`) so the tool description can
    // carry runtime facts: the backend's dialect hints and the row cap.
    async fn list_tools(
        &self,
...
async fn main() -> Result<()> {
    // stdout is the MCP transport; all human-facing logging must go to stderr.
